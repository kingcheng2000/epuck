...
-- regarding blobs detected, such as amount, size, position, etc.
 ...
 -- (if Detection is successful) in t1 we should have the blob information if the camera was set-up correctly  
    if (t1) then        
        blobCount=t1[1]
...
-- PID control section
-- Construct pid table
     local pid = {SetPos, ActualPos, err, err_last,Kp,Ki,Kd,integral}         
...
-- algorithm for PID control
        ...
        pidSpeed=pid.Kp*pid.err+pid.Ki*pid.integral+pid.Kd*(pid.err-pid.err_last)
        ActualSpeed = maxVel-(pid.Kp*pid.err+pid.Ki*pid.integral+pid.Kd*(pid.err-pid.err_last))
       ...
       print(pid.err_last)
        ...
        if (t1[5]<0.45) then
        sim.setJointTargetVelocity(leftMotor,ActualSpeed)
        sim.setJointTargetVelocity(rightMotor,maxVel)
        end

        if (t1[5]>0.55) then
        sim.setJointTargetVelocity(leftMotor,maxVel)
        sim.setJointTargetVelocity(rightMotor,ActualSpeed)
        end
...
--[[
        ...
        if (blobCount == 0)then 
                sim.setJointTargetVelocity(leftMotor,-maxVel*0.7)
                sim.setJointTargetVelocity(rightMotor,maxVel)
        end
